...
public class SmartsParser {
	...
	Vector<IAtom> addLayerToCode(Vector<Integer> code, Vector<IAtom> layer,
			Vector<IAtom> usedAtoms) {
		if (layer == null)
			return null;
	...
	static public Vector<int[]> getRindData(IAtomContainer container,
			IRingSet ringSet) {
	...
	static public Vector<int[]> getRindData2(IAtomContainer container,
			IRingSet ringSet) {
		Vector<int[]> v = new Vector<int[]>();
		IRingSet atomRings;
		for (int i = 0; i < container.getAtomCount(); i++) {
